...
def format_tips_message(match_info: dict, tips: dict, odds_data: dict | None) -> str:
    ...
    source_line   = "📡 Odds: Live bookmaker" if has_real_odds else "🤖 Odds: AI estimate"

    lines = ["⚽️ <b>{} vs {}</b>".format(home, away)]
    if league:
    ...
    for i, bet in enumerate(tips.get("bets", [])):
        ...
        lines.append(
            f"{medal} <b>{bet.get('pick', '')}</b>  ·  {bet.get('market', '')}\n"
            f"   💰 Odds: <code>{float(bet.get('odds', 0)):.2f}</code>\n"
            f"   {bar} <b>{pct}%</b>\n"
            f"   <i>{bet.get('value_reason', '')}</i>\n"
        )

    lines.append("━━━━━━━━━━━━━━━━━━")
    lines.append("⚠️ <i>للمعلومات فقط — تراهن بمسؤولية.</i>")
